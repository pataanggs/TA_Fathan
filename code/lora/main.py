"""
Main entry point for Whisper LoRA fine-tuning on Minangkabau language.
Implements 5-fold cross-validation with PEFT LoRA adapters.
Includes comprehensive metrics logging to both local files and WandB.

Usage:
    cd /path/to/code/lora
    python main.py
"""
...
import config  # This imports lora/config.py
...
def check_peft_installed():
    """Verify PEFT library is installed."""
    try:
        import peft
...
def print_experiment_config():
    ...
    print("\n" + "=" * 70)
    print("EXPERIMENT CONFIGURATION (LoRA)")
    print("=" * 70)
    ...
    print(f"   Modules to save:    {LORA_CONFIG.get('modules_to_save', 'None')}")
    ...
    print(f"   Label Smoothing:    {TRAINING_ARGS.get('label_smoothing_factor', 0)}")
...
def main():
    """Main training pipeline with 5-fold cross-validation using LoRA."""

    print("=" * 70)
    print("WHISPER LoRA FINE-TUNING FOR MINANGKABAU LANGUAGE")
    ...
    # Check PEFT
    if not check_peft_installed():
        print("\n❌ Aborting: PEFT library required for LoRA training.")
    ...
    # ==========================================================================
    # STEP 3: 5-Fold Cross-Validation with LoRA
    # ==========================================================================
    print("\n[STEP 3] Starting 5-Fold Cross-Validation with LoRA...")
    ...
    for fold_idx, (train_indices, eval_indices) in enumerate(kfold.split(range(len(dataset)))):
        ...
        print(f"\n--- Fold {fold_idx + 1}/{NUM_FOLDS} (LoRA) ---")
    ...
    # ==========================================================================
    print("\n" + "=" * 70)
    print("FINAL RESULTS - 5-FOLD CROSS-VALIDATION (LoRA)")
    print("=" * 70)
    ...
    cv_summary = {
        ...
        "training_config": {
            ...
            "label_smoothing_factor": TRAINING_ARGS.get("label_smoothing_factor", 0),
        },
        "model_config": {
