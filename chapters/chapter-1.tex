...
\begin{enumerate}
    ...
    \item \textbf{Menerapkan dan membandingkan strategi fine-tuning yang berbeda}, yaitu \textbf{Freeze Encoder (Decoder-Only Fine-Tuning)} sebagai pendekatan standar yang melatih seluruh \textit{decoder} (37 juta parameter), dan \textit{parameter-efficient fine-tuning} dengan \textbf{LoRA} yang melatih $\sim$1,47\%--2,93\% parameter ($\sim$1,08--2,16 juta parameter, tergantung konfigurasi \textit{rank}) pada seluruh \textit{linear layer} (\texttt{q\_proj}, \texttt{v\_proj}, \texttt{k\_proj}, \texttt{out\_proj}, \texttt{fc1}, \texttt{fc2}). Tujuan perbandingan ini adalah untuk \textbf{mengevaluasi efektivitas} kedua pendekatan pada skenario \textit{extremely low-resource} (dataset 1.3 jam), dengan mempertimbangkan \textit{trade-off} antara akurasi model, waktu pelatihan, kebutuhan memori komputasi, dan potensi risiko \textit{overfitting}.
