...
\begin{enumerate}
    ...
    \item Bagaimana pengaruh penerapan teknik \textit{fine-tuning} tertentu (misalnya \textit{full fine-tuning} atau \textit{parameter-efficient fine-tuning} seperti \textbf{LoRA}) terhadap \textbf{kinerja model Whisper Small} dalam mengenali ucapan bahasa Minangkabau?
    
    \item Bagaimana cara mengukur dan mengevaluasi \textbf{performa model hasil fine-tuning} menggunakan metrik kuantitatif seperti \textit{Word Error Rate (WER)} dan \textit{Character Error Rate (CER)}, serta analisis kualitatif terhadap kesalahan transkripsi pada beragam variasi penutur dan kondisi rekaman?
...
\begin{enumerate}
    ...
    \item \textbf{Menerapkan dan membandingkan teknik fine-tuning yang berbeda}, seperti \textit{full fine-tuning} dan \textit{parameter-efficient fine-tuning} (misalnya \textbf{LoRA-Whisper}), untuk menentukan metode yang paling efektif terhadap bahasa dengan sumber daya terbatas (\textit{low-resource language}).
    
    \item \textbf{Mengevaluasi performa model hasil fine-tuning} menggunakan metrik kuantitatif seperti \textit{Word Error Rate (WER)}, \textit{Character Error Rate (CER)}, dan analisis kualitatif terhadap kesalahan transkripsi, guna mengetahui sejauh mana peningkatan yang dicapai.
...
\begin{enumerate}
    ...
    \item Dataset yang digunakan terdiri dari \textbf{rekaman suara dan transkripsi teks bahasa Minangkabau} yang bersumber dari korpus publik, hasil pengumpulan mandiri, atau dataset hasil \textit{crowdsourcing} berskala terbatas. Penelitian ini tidak mencakup proses pembuatan dataset baru dalam skala besar.
    ...
    \item Evaluasi performa model dilakukan menggunakan metrik \textbf{Word Error Rate (WER)}, \textbf{Character Error Rate (CER)}, dan analisis kesalahan transkripsi pada sampel data uji. Evaluasi subjektif atau berbasis pengguna akhir (\textit{user study}) tidak termasuk dalam ruang lingkup penelitian ini.
