...
\begin{enumerate}
    ...
    \item Bagaimana pengaruh penerapan strategi \textit{fine-tuning} yang berbeda (\textit{Freeze Encoder dengan melatih hanya Decoder} versus \textit{parameter-efficient fine-tuning} dengan \textbf{LoRA}) terhadap \textbf{kinerja model Whisper Small} dalam mengenali ucapan bahasa Minangkabau, terutama dalam konteks dataset yang sangat terbatas (\textit{extremely low-resource}) dan keterbatasan sumber daya komputasi?
    
    \item Bagaimana cara mengukur dan mengevaluasi \textbf{performa model hasil fine-tuning} menggunakan metrik kuantitatif seperti \textit{Word Error Rate (WER)} dan \textit{Character Error Rate (CER)}, serta melakukan analisis kualitatif terhadap kesalahan transkripsi dengan mengkategorikan jenis kesalahan (fonetik, leksikal, dan kontekstual) pada variasi penutur dan kondisi rekaman?
...
\begin{enumerate}
    ...
    \item \textbf{Menerapkan dan membandingkan strategi fine-tuning yang berbeda}, yaitu \textit{Freeze Encoder (melatih hanya Decoder)} dan \textit{parameter-efficient fine-tuning} dengan \textbf{LoRA}, untuk menentukan metode yang paling efektif dan efisien dalam konteks dataset yang sangat terbatas (\textit{extremely low-resource}), dengan mempertimbangkan trade-off antara akurasi, waktu pelatihan, dan kebutuhan memori komputasi.
